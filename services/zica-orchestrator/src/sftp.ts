...
export async function deployStatic(target:TargetRecord,article:ArticlePayload){
  const creds=credential(target.credential_ref);
  ...
  try{
    await sftp.connect({host:creds.host,port:creds.port||22,username:creds.username,...(creds.privateKey?{privateKey:creds.privateKey}:{password:creds.password}),readyTimeout:15000});
    await sftp.mkdir(dir,true);
    await sftp.put(Buffer.from(html,'utf8'),tempFile);
    await sftp.rename(tempFile,targetFile);
  }finally{
