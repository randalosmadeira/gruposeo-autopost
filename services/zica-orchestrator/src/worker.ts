...
import { CONTENT_QUEUE_NAME,INDEX_QUEUE_NAME,SUPPORTER_AVATAR_QUEUE_NAME,enqueueIndexFlush,enqueueReconcile } from './queues.js';
import { processSupporterAvatarJob,type SupporterAvatarJobData } from './supporter-avatar/pipeline.js';
...
// Apoiadores 1470: uma geração de imagem por pedido; lock longo cobre a chamada de imagem (até 180 s) com folga.
const supporterAvatarWorker=new Worker<SupporterAvatarJobData>(SUPPORTER_AVATAR_QUEUE_NAME,async job=>processSupporterAvatarJob(job.data,job.attemptsMade+1),{connection:redis,concurrency:Number(process.env.SUPPORTER_AVATAR_CONCURRENCY||3),lockDuration:420000,limiter:{max:30,duration:60000}});
...
for(const signal of ['SIGTERM','SIGINT'] as const)process.on(signal,async()=>{await Promise.allSettled([contentWorker.close(),indexWorker.close(),supporterAvatarWorker.close()]);await redis.quit();process.exit(0);});
