(() => {
  ...
  root.innerHTML = `
    ...
    <section class="zica-optin-dialog" role="dialog" aria-modal="true" aria-labelledby="zica-optin-title">
      ...
      <form class="zica-optin-form" novalidate>
        <label class="zica-optin-field">
          <span class="zica-optin-icon" aria-hidden="true">♙</span>
          <input name="fullName" autocomplete="name" placeholder="Seu nome completo" maxlength="120" required />
        </label>
        <label class="zica-optin-field">
          <span class="zica-optin-icon" aria-hidden="true">✉</span>
          <input name="email" type="email" autocomplete="email" placeholder="seu@email.com" maxlength="180" />
        </label>
        <label class="zica-optin-field">
          <span class="zica-optin-icon" aria-hidden="true">◉</span>
          <input name="whatsapp" inputmode="tel" autocomplete="tel" placeholder="(11) 99999-9999" maxlength="24" />
        </label>
        <div class="zica-optin-location-row">
          <label class="zica-optin-field">
            <span class="zica-optin-icon" aria-hidden="true">⌖</span>
            <input name="city" autocomplete="address-level2" placeholder="Cidade" maxlength="100" />
          </label>
          <label class="zica-optin-field zica-optin-state-field">
            <select name="state" autocomplete="address-level1" aria-label="UF">
              <option value="">UF</option>
              <option>AC</option><option>AL</option><option>AP</option><option>AM</option><option>BA</option><option>CE</option><option>DF</option><option>ES</option><option>GO</option><option>MA</option><option>MT</option><option>MS</option><option>MG</option><option>PA</option><option>PB</option><option>PR</option><option>PE</option><option>PI</option><option>RJ</option><option>RN</option><option>RS</option><option>RO</option><option>RR</option><option>SC</option><option selected>SP</option><option>SE</option><option>TO</option>
            </select>
          </label>
        </div>
  ...
  const setOpen = (open) => {
    ...
    document.documentElement.classList.toggle('zica-optin-lock', open);
    if (open) {
      window.setTimeout(() => root.querySelector('input[name="fullName"]')?.focus(), 80);
    }
  };

  const openModal = (reason) => {
    if (root.classList.contains('is-open') || submitting || successBox.hidden === false) return;
    if (reason === 'scroll' && sessionStorage.getItem(scrollSeenKey)) return;
    if (reason === 'exit' && sessionStorage.getItem(exitSeenKey)) return;

    if (reason === 'scroll') sessionStorage.setItem(scrollSeenKey, '1');
    if (reason === 'exit') sessionStorage.setItem(exitSeenKey, '1');
    currentReason = reason;
    setOpen(true);
    push('zica_optin_popup_impression', { zica_trigger: reason });
  };

  const closeModal = () => {
    if (!root.classList.contains('is-open')) return;
    setOpen(false);
    const closeCount = Number(sessionStorage.getItem(closeCountKey) || 0) + 1;
    sessionStorage.setItem(closeCountKey, String(closeCount));
    if (closeCount >= 2 || currentReason === 'exit') {
      const hours = Math.max(1, Number(config.dismissHours || 24));
      localStorage.setItem(dismissKey, String(Date.now() + hours * 60 * 60 * 1000));
    }
    push('zica_optin_popup_close', { zica_trigger: currentReason || 'unknown' });
  };
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const emailUpdates = data.get('emailUpdates') === 'on';
    const whatsappUpdates = data.get('whatsappUpdates') === 'on';
    const volunteer = data.get('volunteer') === 'on';
    const consentContact = data.get('consentContact') === 'on';

    if (fullName.length < 2) {
      errorBox.textContent = 'Informe seu nome completo.';
      return;
    }
    if (!email && !whatsapp) {
      errorBox.textContent = 'Informe pelo menos email ou WhatsApp.';
      return;
    }
    if (!emailUpdates && !whatsappUpdates && !volunteer) {
      errorBox.textContent = 'Selecione como deseja participar ou receber contato.';
      return;
    }
    if (!consentContact) {
      errorBox.textContent = 'Confirme a autorização para registrar o cadastro.';
      return;
    }
    if (emailUpdates && !email) {
      errorBox.textContent = 'Informe o email para receber novidades por email.';
      return;
    }
    if (whatsappUpdates && !whatsapp) {
      errorBox.textContent = 'Informe o WhatsApp para receber novidades por WhatsApp.';
      return;
    }
    ...
    try {
      const response = await fetch(config.apiUrl, {
        ...
        body: JSON.stringify({
          ...
          sourcePortal: config.sourcePortal || window.location.hostname,
        }),
      });
      ...
      const days = Math.max(1, Number(config.successSuppressDays || 90));
      localStorage.setItem(successKey, String(Date.now() + days * 24 * 60 * 60 * 1000));
      localStorage.removeItem(dismissKey);
      push('zica_optin_submit_success', { zica_trigger: currentReason || 'direct' });
      window.setTimeout(() => setOpen(false), 1800);
    } catch (error) {
      errorBox.textContent = 'Não foi possível concluir o cadastro agora. Tente novamente.';
      push('zica_optin_submit_error', { zica_trigger: currentReason || 'direct' });
    } finally {
      submitting = false;
      submitButton.disabled = false;
      submitButton.textContent = config.buttonLabel || '🪵 MADEIRAAA NELESS';
    }
  });
  ...
  const onScroll = () => {
    ...
    const depth = Math.min(100, (window.scrollY / scrollable) * 100);
    if (window.scrollY >= 120 && depth >= threshold) {
  ...
  if (config.exitIntentEnabled && window.matchMedia('(pointer:fine)').matches) {
