...
import { Vote, Megaphone, Flag, Scale, Globe, Instagram, Youtube, Twitter, Smartphone, BookOpen, Flame, Send, FileText, Share2, Users, MapPin, Shield, Sparkles, AlertCircle, CheckCircle2, } from 'lucide-react';
const defaultConfig = {
    candidateName: '',
    politicalParty: '',
...
export default function ElectoralCampaign() {
    ...
    const derivedPhase = useMemo(() => {
        ...
        if (today < startCampaign)
            return 'pre-campanha';
        if (today <= endCampaign)
            return 'campanha';
        return 'pos-pleito';
    }, []);
    const updateConfig = (field, value) => setConfig(prev => ({ ...prev, [field]: value }));
    const updateSocial = (field, value) => setConfig(prev => ({ ...prev, socialMedia: { ...prev.socialMedia, [field]: value } }));
    ...
    return (<div className="space-y-6">
      ...
      <Tabs value={currentTab} onValueChange={setCurrentTab} className="space-y-4">
              ...
              <CardContent className="space-y-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 p-2 bg-muted rounded-md text-sm border">
                    {derivedPhase === 'campanha' ? (<CheckCircle2 className="w-4 h-4 text-green-500"/>) : (<AlertCircle className="w-4 h-4 text-orange-500"/>)}
        ...
        <TabsContent value="suggestions">
          <AISuggestionsPanel candidateRole={config.candidateRole} candidateName={config.candidateName} city={config.city || selectedCities[0] || 'São Paulo'} onSelectKeyword={(kw) => setKeyword(kw)} onSelectTopics={setSelectedTopics} selectedTopics={selectedTopics}/>
          <div className="mt-4 flex justify-between">
          ...
          <div className="grid md:grid-cols-3 gap-4">
             <div className="md:col-span-2 space-y-4">
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div>
                      ...
                      <Input placeholder="Ex: CNH aos 16 anos em São Paulo" value={keyword} onChange={e => setKeyword(e.target.value)}/>
                    </div>
                    <div className="flex items-center space-x-2">
                       <input type="checkbox" id="indexnow" checked={config.notifyIndexNow} onChange={e => updateConfig('notifyIndexNow', e.target.checked)} className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary"/>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {contentTemplates.map(t => (<Card key={t.id} className={`cursor-pointer border-2 transition-all ${selectedTemplate === t.id ? 'border-primary bg-primary/5' : 'hover:border-primary/50'}`} onClick={() => setSelectedTemplate(t.id)}>
             ...
             <Card className="h-fit">
                ...
                <CardContent className="space-y-4">
                  {isGenerating ? (<div className="space-y-2">
                    ...
                    </div>) : (<Button className="w-full h-12 bg-primary text-white font-bold" onClick={handleGenerate} disabled={!keyword || !config.candidateName}>
