...
export default function Auth() {
  ...
  if (loading) return <div className="flex min-h-screen items-center justify-center bg-[#050a0f]"><div className="zica-auth-loading flex items-center gap-3 rounded-2xl border border-[#263541] bg-[#0b141d] px-5 py-4 text-sm text-slate-300"><Loader2 className="h-5 w-5 animate-spin text-[#D4FF00]" /> Sincronizando o Zica.IA Posts...</div></div>;
  ...
  const healthLabel = backendStatus === 'online' ? `Zica.IA Posts ativo${latency ? ` · ${latency}ms` : ''}` : backendStatus === 'degraded' ? `Sistema com latência${latency ? ` · ${latency}ms` : ''}` : backendStatus === 'offline' ? 'Sistema temporariamente indisponível' : 'Verificando Zica.IA Posts';
  ...
  return (
    <div className="zica-auth-v2 relative min-h-screen overflow-hidden bg-[#050a0f] text-slate-100">
      ...
      <div className="relative z-10 mx-auto flex min-h-screen max-w-[1680px] flex-col px-4 py-5 sm:px-6 lg:px-8 xl:px-10">
        ...
        <main className="grid flex-1 items-center gap-8 py-8 lg:grid-cols-[1.62fr_.78fr] xl:gap-10">
          ...
          <section className="relative z-30 mx-auto w-full max-w-[500px]">
            <div className="zica-login-panel relative overflow-hidden rounded-[26px] border border-[#00F0FF]/35 bg-[#071018]/90 p-5 shadow-[0_30px_100px_rgba(0,0,0,.55),0_0_70px_rgba(0,240,255,.035)] backdrop-blur-xl sm:p-7 xl:p-8">
              ...
              <div className="mb-7 flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl border border-[#D4FF00]/35 bg-[#D4FF00]/[.05] text-[#D4FF00]"><LockKeyhole className="h-5 w-5" /></span><div><p className="text-xs font-black uppercase tracking-[.15em] text-[#D4FF00]">{view === 'login' ? 'Acesso ao Zica.IA Posts' : view === 'signup' ? 'Ativar novo ecossistema' : 'Recuperar acesso'}</p><p className="mt-1 text-xs text-slate-500">Zica.IA Posts · conexão criptografada</p></div></div>
        ...
        <footer className="mx-auto flex w-full max-w-5xl items-center gap-4 rounded-full border border-[#1f303b] bg-[#071018]/70 px-5 py-3 text-[10px] font-bold uppercase tracking-[.12em] text-slate-500 backdrop-blur sm:text-xs"><span className="h-2 w-2 animate-pulse rounded-full bg-[#3cff65] shadow-[0_0_12px_#3cff65]" /><strong className="text-[#D4FF00]">Zica.IA Posts ativo</strong><span className="hidden sm:inline">Monitorando, aprendendo e impulsionando seu tráfego 24/7</span><span className="zica-footer-wave ml-auto hidden w-36 sm:block" /></footer>
      </div>
    </div>
  );
}
