...
class Zica_AI_Connector_Core {
    ...
    public function output_json_ld_schemas() {
        ...
        echo "\n<!-- JSON-LD Structured Data by Zica.ai RDM -->\n";
...
private function init_admin_hooks() {
    ...
    public function show_diagnostics_notice() {
        ...
        $title = ($status === 'critical') ? 'Zica.ai: Diagnóstico CRÍTICO' : 'Zica.ai: Avisos de Diagnóstico';
    ...
    public function handle_ajax_repair_tables() {
        ...
        if (empty(get_option('zica_ai_api_key'))) {
            update_option('zica_ai_api_key', wp_generate_uuid4());
    ...
    public function enqueue_admin_assets($hook) {
        ...
        wp_localize_script('zica-ai-admin', 'cfrdmAdmin', array(
            ...
            'apiKey' => get_option('zica_ai_api_key'),
            'siteUrl' => get_site_url(),
            'strings' => array(
                'testSuccess' => __('Conexão estabelecida com sucesso!', 'zica-ai'),
                'testError' => __('Erro ao testar conexão.', 'zica-ai'),
                'copied' => __('Copiado!', 'zica-ai'),
                'confirm_regenerate' => __('Tem certeza que deseja regenerar a API Key? Você precisará atualizar a configuração no Zica.ai.', 'zica-ai'),
                'confirm_clear_logs' => __('Tem certeza que deseja limpar todos os logs?', 'zica-ai'),
                'logs_cleared' => __('Logs limpos com sucesso!', 'zica-ai'),
                'syncing' => __('Sincronizando...', 'zica-ai'),
                'sync_complete' => __('Sincronização concluída!', 'zica-ai'),
                'auto_correcting' => __('Executando autocorreções...', 'zica-ai'),
                'auto_correct_complete' => __('Autocorreções concluídas!', 'zica-ai'),
            ),
        ));
    }
    ...
    public function register_settings() {
        // Connection settings
        register_setting('zica_ai_settings', 'cfrdm_enabled');
        register_setting('zica_ai_settings', 'cfrdm_api_url');
        register_setting('zica_ai_settings', 'zica_ai_api_key');
        
        // Webhook settings
        register_setting('zica_ai_settings', 'cfrdm_webhook_enabled');
        register_setting('zica_ai_settings', 'cfrdm_webhook_secret');
        
        // Publishing settings
        register_setting('zica_ai_settings', 'cfrdm_auto_publish');
        register_setting('zica_ai_settings', 'cfrdm_default_status');
        register_setting('zica_ai_settings', 'cfrdm_default_category');
        register_setting('zica_ai_settings', 'cfrdm_default_author');
        
        // Image settings
        register_setting('zica_ai_settings', 'cfrdm_auto_optimize_images');
        register_setting('zica_ai_settings', 'cfrdm_image_max_width');
        register_setting('zica_ai_settings', 'cfrdm_image_quality');
        
        // Auto-correction settings
        register_setting('zica_ai_settings', 'cfrdm_auto_correct');
        register_setting('zica_ai_settings', 'cfrdm_auto_correct_seo');
        register_setting('zica_ai_settings', 'cfrdm_auto_correct_images');
        register_setting('zica_ai_settings', 'cfrdm_auto_correct_links');
        
        // Logging settings
        register_setting('zica_ai_settings', 'cfrdm_log_retention_days');
        register_setting('zica_ai_settings', 'cfrdm_log_api_calls');
        register_setting('zica_ai_settings', 'cfrdm_log_webhooks');
        
        // v3.0.0 - GSC Settings
        register_setting('zica_ai_settings', 'cfrdm_gsc_client_id');
        register_setting('zica_ai_settings', 'cfrdm_gsc_client_secret');
        register_setting('zica_ai_settings', 'cfrdm_gsc_site_url');
        
        // v3.0.0 - AI Auto-Fix Settings
        register_setting('zica_ai_settings', 'cfrdm_ai_auto_fix_enabled');
        register_setting('zica_ai_settings', 'cfrdm_ai_auto_fix_min_confidence');
        register_setting('zica_ai_settings', 'cfrdm_content_enhancer_enabled');
        register_setting('zica_ai_settings', 'cfrdm_https_enforcer_enabled');
        
        // v3.0.0 - Auto-Update Settings
        register_setting('zica_ai_settings', 'cfrdm_auto_update_enabled');
        
        // v3.1.0 - SEO Discovery Settings
        register_setting('zica_ai_settings', 'cfrdm_meta_auditor_enabled');
        register_setting('zica_ai_settings', 'cfrdm_meta_auditor_batch_size');
        register_setting('zica_ai_settings', 'cfrdm_indexnow_enabled');
        register_setting('zica_ai_settings', 'cfrdm_google_ping_enabled');
        register_setting('zica_ai_settings', 'cfrdm_bing_ping_enabled');
        register_setting('zica_ai_settings', 'cfrdm_llms_txt_enabled');
        register_setting('zica_ai_settings', 'cfrdm_sitemap_optimizer_enabled');
    }
...
add_action('plugins_loaded', function() {
    Zica_AI_Connector_Core::get_instance();
}, 5);
