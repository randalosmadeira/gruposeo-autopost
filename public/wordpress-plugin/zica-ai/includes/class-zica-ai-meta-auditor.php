...
class ZICA_AI_Meta_Auditor {
    ...
    public function run_audit() {
        ...
        if ($stats['meta_fixed'] > 0 || $stats['og_fixed'] > 0) {
            ZICA_AI_Logger::success('meta_auditor', 'Auditoria SEO concluída', $stats);
        }
    ...
    public function audit_single_post($post) {
        ...
        if (class_exists('ZICA_AI_SEO')) {
            $current_meta = ZICA_AI_SEO::get_meta($post->ID);
        }
        ...
        if ($needs_ai) {
            ...
            // Apply via ZICA_AI_SEO
            if (!empty($seo_meta) && class_exists('ZICA_AI_SEO')) {
                ZICA_AI_SEO::set_meta($post->ID, $seo_meta);
            }
            
            // Notify search engines about the update via IndexNow
            if (($result['meta_fixed'] || $result['og_fixed']) && class_exists('ZICA_AI_IndexNow')) {
                $permalink = get_permalink($post->ID);
                if ($permalink) {
                    ZICA_AI_IndexNow::get_instance()->submit_indexnow($permalink);
                    ZICA_AI_Logger::info('meta_auditor', 'IndexNow notificado após correção de meta', array(
                        'post_id' => $post->ID,
                        'url' => $permalink,
                    ), $post->ID);
                }
    ...
    private function generate_seo_meta_via_ai($post, $missing_fields) {
        ...
        if (class_exists('ZICA_AI_AI_SEO')) {
            $reflection = new ReflectionMethod('ZICA_AI_AI_SEO', 'call_ai_api');
            ...
            $reflection2 = new ReflectionMethod('ZICA_AI_AI_SEO', 'extract_json');
