...
class ZICA_AI_Sync {
    ...
    /**
     * Sync stats to Zica.ai platform
     */
    public static function sync_stats_to_platform() {
        ...
        if (is_wp_error($response)) {
            ZICA_AI_Logger::error('sync', 'Falha ao sincronizar estatísticas', array(
                'error' => $response->get_error_message(),
            ));
        } else {
            ZICA_AI_Logger::debug('sync', 'Estatísticas sincronizadas', $stats);
        }
    }
    ...
    public static function get_full_stats() {
        $article_stats = ZICA_AI_Articles::get_stats();
    ...
    public static function get_recent_sync_activity($limit = 10) {
        ...
        $table = $wpdb->prefix . ZICA_AI_LOG_TABLE;
    ...
    public static function save_news($news) {
        ...
        $table = $wpdb->prefix . ZICA_AI_NEWS_TABLE;
    ...
    public static function auto_correct_post($post_id, $post, $update) {
        ...
        if (!empty($corrections)) {
            ZICA_AI_Logger::warning('autocorrect', 'Problemas detectados no artigo', array(
                'post_id' => $post_id,
    ...
    public static function run_bulk_autocorrect($limit = 50) {
        ...
        ZICA_AI_Logger::info('autocorrect', 'Autocorreção em massa executada', $results);
