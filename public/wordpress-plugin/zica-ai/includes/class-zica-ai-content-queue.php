...
class ZICA_AI_Content_Queue {
    ...
    public static function create_table() {
        ...
        ZICA_AI_Logger::success('content_queue', 'Tabela de fila de conteúdo criada/atualizada');
    }
    ...
    public static function push($queue_type, $action, $payload, $options = array()) {
        ...
        if ($result === false) {
            ZICA_AI_Logger::error('content_queue', 'Falha ao adicionar item à fila', array(
        ...
        ZICA_AI_Logger::debug('content_queue', 'Item adicionado à fila', array(
            'id' => $queue_id,
            'type' => $queue_type,
            'action' => $action,
    ...
    public static function fail($id, $error_message, $error_trace = null) {
        ...
        // Log the failure
        ZICA_AI_Logger::error('content_queue', 'Item falhou', array(
            'id' => $id,
            'action' => $item['action'],
    ...
    public static function cleanup($days = 7) {
        ...
        if ($deleted > 0) {
            ZICA_AI_Logger::info('content_queue', "Limpeza: {$deleted} itens removidos da fila");
        }
    ...
    public static function reset_stuck($timeout_minutes = 30) {
        ...
        if ($reset > 0) {
            ZICA_AI_Logger::warning('content_queue', "{$reset} itens travados foram resetados");
        }
