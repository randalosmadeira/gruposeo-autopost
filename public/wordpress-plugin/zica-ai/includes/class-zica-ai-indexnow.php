...
class ZICA_AI_IndexNow {
    ...
    public function on_post_publish($post_id, $post) {
        ...
        if (class_exists('ZICA_AI_Meta_Auditor')) {
            try {
                if (ZICA_AI_Method_Validator::safe_call('ZICA_AI_Meta_Auditor', 'is_enabled', array(), false)) {
                    $auditor = ZICA_AI_Meta_Auditor::get_instance();
                    $auditor->audit_single_post($post_id);
                }
            } catch (\Throwable $e) {
                ZICA_AI_Logger::warning('indexnow', 'Meta audit falhou: ' . $e->getMessage());
            }
        }
        ...
        if (class_exists('ZICA_AI_LLMS_Txt')) {
            try {
                ...
                ZICA_AI_Logger::warning('indexnow', 'LLMS cache invalidation falhou: ' . $e->getMessage());
            }
        }
        
        ZICA_AI_Logger::success('indexnow', 'URL submetida para indexação + meta auditada', array(
            'url' => $url,
            'post_id' => $post_id,
        ), $post_id);
    }
    ...
    public function submit_indexnow($url, $attempt = 0) {
        ...
        if ($count >= 10) {
            ...
            ZICA_AI_Logger::info('indexnow', 'URL enfileirada (rate limit local)', array('url' => $url));
            return true;
        }
        ...
        if (is_wp_error($response)) {
            ZICA_AI_Logger::error('indexnow', 'Erro ao submeter IndexNow', array(
        ...
        if ($status === 429 && $attempt < 3) {
            ...
            ZICA_AI_Logger::warning('indexnow', "429 recebido, retry #{$attempt} em {$delay}s", array('url' => $url));
            sleep($delay);
        ...
        ZICA_AI_Logger::warning('indexnow', "IndexNow retornou status {$status}", array('url' => $url, 'status' => $status));
        return false;
    }
