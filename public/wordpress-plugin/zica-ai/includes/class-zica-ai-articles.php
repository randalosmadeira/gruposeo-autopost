...
class ZICA_AI_Articles {
    ...
    public static function get_stats() {
        ...
        // Count synced articles (with Zica.ai ID)
    ...
    /**
     * Sync article from Zica.ai
     */
    public static function sync_from_contentfactory($zica_ai_id, $article_data) {
        // Check if article already exists
        $existing = self::get_by_cfrdm_id($zica_ai_id);
        ...
        if (is_wp_error($post_id)) {
            ZICA_AI_Logger::error('sync', 'Erro ao sincronizar artigo', array(
                'cfrdm_id' => $zica_ai_id,
        ...
        // Set Zica.ai ID
        update_post_meta($post_id, '_cfrdm_article_id', sanitize_text_field($zica_ai_id));
        ...
        ZICA_AI_Logger::success('sync', "Artigo $action via sincronização", array(
            'post_id' => $post_id,
            'cfrdm_id' => $zica_ai_id,
    ...
    public static function set_sync_error($post_id, $error_message) {
        ...
        ZICA_AI_Logger::error('sync', $error_message, array(
            'post_id' => $post_id,
        ), $post_id);
    }
    ...
    /**
     * Get article by Zica.ai ID
     */
    public static function get_by_cfrdm_id($zica_ai_id) {
        global $wpdb;
    ...
    public static function bulk_import($articles) {
        ...
        foreach ($articles as $article) {
            if (empty($article['cfrdm_id'])) {
                $results['failed']++;
                $results['errors'][] = __('ID do Zica.ai não fornecido', 'zica-ai');
                continue;
            }
        ...
        ZICA_AI_Logger::info('sync', 'Importação em massa concluída', $results);
