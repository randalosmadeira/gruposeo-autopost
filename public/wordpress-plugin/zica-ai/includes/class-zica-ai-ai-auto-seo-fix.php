...
class ZICA_AI_AI_Auto_SEO_Fix {
    ...
    public static function register_routes() {
        ...
        register_rest_route('zica-ai/v1', '/scan-404-urls', array(
            'methods' => 'POST',
            'callback' => array(__CLASS__, 'scan_404_urls'),
            'permission_callback' => array('ZICA_AI_API', 'verify_api_key'),
        ));

        register_rest_route('zica-ai/v1', '/auto-fix-404', array(
            'methods' => 'POST',
    ...
    public static function update_seo_meta($request) {
        ...
        ZICA_AI_Logger::success('ai_auto_seo', "SEO meta atualizado para post {$post_id}", array(
            'plugin' => $seo_plugin,
    ...
    public static function auto_fix_404($request) {
        ...
        foreach ($redirects as $redirect) {
            ...
            if (class_exists('ZICA_AI_Redirect_Manager')) {
                $result = ZICA_AI_Redirect_Manager::add_redirect($source, $target, 301, $notes);
                if ($result) {
                    $created++;
                } else {
        ...
        if ($created > 0) {
            ZICA_AI_Logger::success('ai_auto_seo', "{$created} redirects 301 criados automaticamente", array(
