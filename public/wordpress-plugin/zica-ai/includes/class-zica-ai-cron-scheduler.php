...
class ZICA_AI_Cron_Scheduler {
    ...
    public static function create_tables() {
        ...
        ZICA_AI_Logger::success('cron_scheduler', 'Tabelas de agendamento criadas/atualizadas');
    }
    ...
    public static function run_job($job_name, $manual = false) {
        ...
        try {
            ...
            ZICA_AI_Logger::error('cron_scheduler', 'Job falhou: ' . $job_name, array(
                'error' => $e->getMessage(),
    ...
    public static function cleanup_history($days = 30) {
        ...
        if ($deleted > 0) {
            ZICA_AI_Logger::info('cron_scheduler', "Histórico limpo: {$deleted} registros removidos");
        }
    ...
    public static function reset_stuck_jobs($timeout_minutes = 30) {
        ...
        foreach ($stuck as $entry) {
            ...
            ZICA_AI_Logger::warning('cron_scheduler', 'Job travado resetado: ' . $entry['job_name']);
        }
        ...
        if ($truncated > 0) {
            ZICA_AI_Logger::info('cron_scheduler', "Histórico truncado reparado: {$truncated} registros");
        }
        ...
        if ($auto_recovered > 0) {
            ZICA_AI_Logger::success('cron_scheduler', "Jobs auto-recuperados: {$auto_recovered}");
        }
    ...
    private static function clear_stale_maintenance_locks() {
        ...
        if (file_exists($maintenance_file)) {
            ...
            if ($mtime && (time() - $mtime) > 900) {
                ...
                ZICA_AI_Logger::warning('cron_scheduler', 'Arquivo .maintenance travado removido (>15min)');
            }
        }
    }
    ...
    public static function run_self_healing() {
        ...
        if (!$exists) {
            ...
            ZICA_AI_Logger::warning('cron_scheduler', 'Tabelas de cron recriadas pelo self-healing');
        }
        ...
        if (!empty($missing)) {
            ...
            ZICA_AI_Logger::info('cron_scheduler', 'Jobs faltantes restaurados: ' . implode(', ', $missing));
        }
