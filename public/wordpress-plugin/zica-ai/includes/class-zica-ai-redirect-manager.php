...
class ZICA_AI_Redirect_Manager {
    ...
    public function register_routes() {
        register_rest_route('zica-ai/v1', '/redirects', array(
            array(
                'methods' => 'GET',
        ...
        register_rest_route('zica-ai/v1', '/redirects/batch', array(
            'methods' => 'POST',
            'callback' => array($this, 'api_batch_redirects'),
            'permission_callback' => array($this, 'check_api_permission'),
        ));
        
        register_rest_route('zica-ai/v1', '/redirects/cleanup-duplicates', array(
            'methods' => 'POST',
            'callback' => array($this, 'api_cleanup_duplicates'),
            'permission_callback' => array($this, 'check_api_permission'),
        ));
        
        register_rest_route('zica-ai/v1', '/redirects/(?P<id>\d+)', array(
            array(
                'methods' => 'DELETE',
        ...
        register_rest_route('zica-ai/v1', '/redirects/stats', array(
            'methods' => 'GET',
    ...
    public function api_create_redirect($request) {
        ...
        ZICA_AI_Logger::success('redirects', 'Redirect 301 criado', array(
            'source' => $source,
            'target' => $target,
            'category' => $category,
        ));
    ...
    public function api_batch_redirects($request) {
        ...
        ZICA_AI_Logger::success('redirects', "Batch: {$created} redirects criados", array(
            'created' => $created,
            'skipped' => $skipped,
        ));
    ...
    public function api_cleanup_duplicates($request) {
        ...
        foreach ($suffix_posts as $post) {
            ...
            if ($original) {
                ...
                if (!$dry_run) {
                    ...
                    ZICA_AI_SEO::set_robots($post->ID, array('noindex' => true, 'nofollow' => false), ZICA_AI_SEO::detect_seo_plugin());
                    $noindex_applied++;
                }
            }
        }
        ...
        foreach ($junk_slugs as $slug) {
            ...
            if ($junk_post) {
                ...
                if (!$dry_run) {
                    ...
                    ZICA_AI_SEO::set_robots($junk_post->ID, array('noindex' => true, 'nofollow' => true), ZICA_AI_SEO::detect_seo_plugin());
                    $noindex_applied++;
                }
            }
        }
        ...
        ZICA_AI_Logger::success('redirects', "Cleanup: {$redirects_created} redirects, {$noindex_applied} noindex", array(
