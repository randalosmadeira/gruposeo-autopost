<?php
/**
 * Indexing and SEO Schema for Zica.ai
 * 
...
class ZICA_AI_Indexing {
    ...
    public static function register_routes() {
        register_rest_route('zica-ai/v1', '/indexing/status', array(
            'methods' => 'GET',
            'callback' => array(__CLASS__, 'get_indexing_status'),
            'permission_callback' => array('ZICA_AI_API', 'verify_api_key'),
        ));
        
        register_rest_route('zica-ai/v1', '/indexing/request', array(
            'methods' => 'POST',
    ...
    public static function add_ai_disclosure_meta() {
        ...
        echo '<meta name="generator" content="Zica.ai AI">' . "\n";
    ...
    public static function notify_search_engines($post_id, $post) {
        ...
        ZICA_AI_Logger::info('indexing', 'Notificação de indexação enviada', array(
            'post_id' => $post_id,
            'url' => $permalink,
        ), $post_id);
    ...
    public static function request_indexing($request) {
        ...
        if (!$post || $post->post_status !== 'publish') {
            return new WP_Error(
                'invalid_post',
                __('Artigo não encontrado ou não publicado.', 'zica-ai'),
                array('status' => 404)
            );
        }
        ...
        return new WP_REST_Response(array(
            'success' => true,
            'message' => __('Solicitação de indexação enviada.', 'zica-ai'),
            'post_id' => $post_id,
    ...
    private static function extract_products_from_content($post) {
        ...
        // First, try to get products from meta (if stored by Zica.ai)
