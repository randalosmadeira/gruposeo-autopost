...
class ZICA_AI_GSC_Integration {
    ...
    public function init() {
        ...
        // Schedule every 6 hours (using custom interval from ZICA_AI_Cron_Scheduler)
    ...
    public function get_auth_url() {
        ...
        $redirect_uri = admin_url('admin.php?page=zica-ai-gsc-auth');
    ...
    public function handle_oauth_callback() {
        if (!isset($_GET['page']) || $_GET['page'] !== 'zica-ai-gsc-auth') {
            return;
        }
        ...
        if ($result) {
            wp_redirect(admin_url('admin.php?page=zica-ai-settings&gsc_connected=1'));
        } else {
            wp_redirect(admin_url('admin.php?page=zica-ai-settings&gsc_error=1'));
        }
        exit;
    }
    ...
    private function exchange_code_for_token($code) {
        ...
        if (is_wp_error($response)) {
            ZICA_AI_Logger::error('gsc_auth', 'Erro ao trocar código por token', array(
        ...
        if (isset($body['access_token'])) {
            ...
            ZICA_AI_Logger::success('gsc_auth', 'GSC conectado com sucesso');
            return true;
        }
        
        ZICA_AI_Logger::error('gsc_auth', 'Token não recebido', $body);
        return false;
    }
    ...
    private function refresh_token($refresh_token) {
        ...
        if (is_wp_error($response)) {
            ZICA_AI_Logger::error('gsc_auth', 'Erro ao renovar token', array(
                'error' => $response->get_error_message(),
            ));
            return null;
        }
    ...
    public function get_404_pages() {
        ...
        if (is_wp_error($response)) {
            ZICA_AI_Logger::error('gsc_api', 'Erro ao buscar 404s', array(
                'error' => $response->get_error_message(),
            ));
            return array();
        }
    ...
    public function request_indexing($url) {
        ...
        if (is_wp_error($response)) {
            ZICA_AI_Logger::error('gsc_indexing', 'Erro ao solicitar indexação', array(
                'url' => $url,
                'error' => $response->get_error_message(),
            ));
            return false;
        ...
        ZICA_AI_Logger::success('gsc_indexing', 'Indexação solicitada', array('url' => $url));
        return true;
    }
    ...
    public function sync_gsc_data() {
        ...
        ZICA_AI_Logger::info('gsc_sync', 'Iniciando sincronização GSC');
        
        // Fetch 404 pages
        $errors_404 = $this->get_404_pages();
        
        // Add to fix queue
        if (!empty($errors_404) && class_exists('ZICA_AI_AI_Auto_Fix')) {
            foreach ($errors_404 as $error) {
                ZICA_AI_AI_Auto_Fix::add_to_queue(
                    '404',
                    $error['url'],
                    null,
                    8, // High priority
        ...
        if (!empty($non_indexed) && class_exists('ZICA_AI_AI_Auto_Fix')) {
        ...
        if (!empty($schema_issues) && class_exists('ZICA_AI_AI_Auto_Fix')) {
        ...
        ZICA_AI_Logger::success('gsc_sync', 'Sincronização GSC concluída', array(
    ...
    public function disconnect() {
        ...
        ZICA_AI_Logger::info('gsc_auth', 'GSC desconectado');
    }
