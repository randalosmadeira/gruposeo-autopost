...
class ZICA_AI_Webhooks {
    ...
    public static function on_post_status_change($new_status, $old_status, $post) {
        ...
        if ($new_status === 'publish') {
            $schema_validation = ZICA_AI_Schema_Validator::validate_post_schemas($post->ID);
        }
    ...
    public static function send_webhook($event, $data) {
        ...
        // Get webhook secret
        $secret = get_option('cfrdm_webhook_secret', get_option('zica_ai_api_key'));
    ...
    public static function test_webhook() {
        return self::send_webhook('test', array(
            'message' => __('Teste de conexão webhook', 'zica-ai'),
            'timestamp' => current_time('c'),
        ));
    }
}
