...
class ZICA_AI_Media {
    ...
    public static function import_image($image_url, $post_id = 0, $options = array()) {
        if (empty($image_url)) {
            return new WP_Error('empty_url', __('URL da imagem não fornecida.', 'zica-ai'));
        }
        ...
        if ($existing) {
            ZICA_AI_Logger::info('media', 'Imagem duplicada detectada - usando existente', array(
                'url_hash' => $url_hash,
        ...
        if (is_wp_error($download_result)) {
            ZICA_AI_Logger::error('media', 'Falha ao baixar imagem após retentativas', array(
                'url' => $image_url,
        ...
        ZICA_AI_Logger::success('media', 'Imagem importada com sucesso', array(
    ...
    public static function download_with_retry($url) {
        ...
        for ($attempt = 1; $attempt <= self::MAX_RETRIES; $attempt++) {
            ...
            // Log retry attempt
            ZICA_AI_Logger::warning('media', sprintf(
    ...
    public static function replace_external_images($content, $post_id, $options = array()) {
        ...
        // Log summary
        if ($stats['found'] > 0) {
            ZICA_AI_Logger::info('media', 'Substituição de imagens externas concluída', array_merge(
                $stats,
    ...
    public static function upload_base64($base64_data, $post_id = 0, $options = array()) {
        ...
        if ($decoded === false) {
            return new WP_Error('invalid_data', __('Dados base64 inválidos.', 'zica-ai'));
        }
    ...
    public static function convert_to_webp($attachment_id, $file_path = null) {
        ...
        try {
            ...
            if ($converted && file_exists($webp_path)) {
                ...
                if (!is_wp_error($webp_id)) {
                    ...
                    if (class_exists('ZICA_AI_Logger')) {
                        ZICA_AI_Logger::success('media', 'Imagem convertida para WebP', array(
            ...
            if (class_exists('ZICA_AI_Logger')) {
                ZICA_AI_Logger::error('media', 'Falha ao converter para WebP: ' . $e->getMessage());
            }
        }
