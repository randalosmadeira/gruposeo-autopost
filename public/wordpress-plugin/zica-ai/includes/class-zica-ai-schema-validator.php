...
class ZICA_AI_Schema_Validator {
    ...
    public static function register_routes() {
        register_rest_route('zica-ai/v1', '/schema/validate', array(
            'methods' => 'POST',
            'callback' => array(__CLASS__, 'validate_schema_endpoint'),
            'permission_callback' => array('ZICA_AI_API', 'verify_api_key'),
        ));
        
        register_rest_route('zica-ai/v1', '/schema/preview', array(
            'methods' => 'POST',
            'callback' => array(__CLASS__, 'preview_schema_endpoint'),
            'permission_callback' => array('ZICA_AI_API', 'verify_api_key'),
        ));
        
        register_rest_route('zica-ai/v1', '/schema/test-url', array(
            'methods' => 'GET',
    ...
    public static function validate_schema_endpoint($request) {
        ...
        return new WP_Error(
            'missing_data',
            __('Forneça post_id ou schema para validar.', 'zica-ai'),
            array('status' => 400)
        );
    }
    ...
    public static function preview_schema_endpoint($request) {
        ...
        if (!$post_id) {
            return new WP_Error(
                'missing_post_id',
                __('ID do artigo não fornecido.', 'zica-ai'),
                array('status' => 400)
            );
        }
    ...
    public static function validate_post_schemas($post_id) {
        ...
        if (!$post) {
            return array(
                'success' => false,
                'error' => __('Artigo não encontrado.', 'zica-ai'),
            );
        }
    ...
    private static function generate_schemas_for_post($post) {
        ...
        // Check if Zica.ai article
        $zica_ai_id = get_post_meta($post->ID, '_cfrdm_article_id', true);
        if (empty($zica_ai_id)) {
            return $schemas;
        }
    ...
    public static function validate_schema($schema) {
        ...
        if (!is_array($schema)) {
            $result['valid'] = false;
            $result['errors'][] = __('Schema deve ser um objeto válido.', 'zica-ai');
            return $result;
        }
        ...
        if (empty($schema['@context'])) {
            $result['valid'] = false;
            $result['errors'][] = __('@context é obrigatório.', 'zica-ai');
        } elseif ($schema['@context'] !== 'https://schema.org') {
            $result['warnings'][] = __('@context deve ser "https://schema.org".', 'zica-ai');
        }
        
        // Check @type
        if (empty($schema['@type'])) {
            $result['valid'] = false;
            $result['errors'][] = __('@type é obrigatório.', 'zica-ai');
            return $result;
        }
        ...
        if (isset(self::$required_fields[$type])) {
            ...
            foreach ($rules['required'] as $field) {
                ...
                if (empty($schema[$field])) {
                    ...
                    $result['errors'][] = sprintf(
                        __('Campo obrigatório "%s" está vazio ou ausente.', 'zica-ai'),
                        $field
                    );
                }
            }
            ...
            foreach ($rules['recommended'] as $field) {
                if (empty($schema[$field])) {
                    $result['warnings'][] = sprintf(
                        __('Campo recomendado "%s" está ausente. Adicionar pode melhorar rich results.', 'zica-ai'),
                        $field
                    );
                }
            }
            ...
            $result['info'][] = sprintf(
                __('Tipo "%s" não tem regras de validação definidas.', 'zica-ai'),
                $type
            );
        }
        ...
        foreach (self::$field_rules as $field => $rules) {
            ...
            if (isset($rules['max_length']) && is_string($value)) {
                if (strlen($value) > $rules['max_length']) {
                    $result['warnings'][] = sprintf(
                        __('Campo "%s" excede %d caracteres (tem %d). Google pode truncar.', 'zica-ai'),
                        $field,
                        $rules['max_length'],
                        strlen($value)
                    );
            ...
            if (isset($rules['min_length']) && is_string($value)) {
                if (strlen($value) < $rules['min_length']) {
                    $result['warnings'][] = sprintf(
                        __('Campo "%s" tem menos de %d caracteres. Considere expandir.', 'zica-ai'),
                        $field,
                        $rules['min_length']
                    );
                }
            ...
            if (isset($rules['format']) && $rules['format'] === 'iso8601') {
                if (!self::is_valid_iso8601($value)) {
                    $result['errors'][] = sprintf(
                        __('Campo "%s" deve estar no formato ISO 8601 (ex: 2024-01-15T10:30:00+00:00).', 'zica-ai'),
                        $field
                    );
                }
            }
            ...
            if (isset($rules['type']) && $rules['type'] === 'numeric') {
                if (!is_numeric($value)) {
                    $result['errors'][] = sprintf(
                        __('Campo "%s" deve ser numérico.', 'zica-ai'),
                        $field
                    );
                } else {
                    if (isset($rules['min']) && $value < $rules['min']) {
                        $result['errors'][] = sprintf(
                            __('Campo "%s" deve ser maior ou igual a %s.', 'zica-ai'),
                            $field,
                            $rules['min']
                        );
                    }
                    if (isset($rules['max']) && $value > $rules['max']) {
                        $result['errors'][] = sprintf(
                            __('Campo "%s" deve ser menor ou igual a %s.', 'zica-ai'),
                            $field,
                            $rules['max']
                        );
                    }
    ...
    public static function validate_on_save($post_id, $post, $update) {
        ...
        // Only for Zica.ai articles
        $zica_ai_id = get_post_meta($post_id, '_cfrdm_article_id', true);
        if (empty($zica_ai_id)) {
            return;
        }
        ...
        if (!$validation['valid']) {
            ZICA_AI_Logger::warning('schema', 'Problemas de validação de schema detectados', array(
                'post_id' => $post_id,
    ...
    public static function show_validation_notices() {
        ...
        if ($error_count > 0) {
            ...
            echo __('Testar no Google', 'zica-ai');
            echo '</a></p></div>';
        }
