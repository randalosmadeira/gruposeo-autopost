...
class ZICA_AI_SEO_Checklist {
    ...
    public function register_routes() {
        register_rest_route('zica-ai/v1', '/seo-checklist', array(
            'methods' => 'GET',
            'callback' => array($this, 'get_checklist'),
            'permission_callback' => function() {
                return ZICA_AI_API::verify_api_key();
            },
        ));
    }
    ...
    private static function check_llms_txt() {
        $enabled = class_exists('ZICA_AI_LLMS_Txt') && ZICA_AI_LLMS_Txt::is_enabled();
    ...
    private static function check_indexnow() {
        $enabled = class_exists('ZICA_AI_IndexNow') && ZICA_AI_IndexNow::is_enabled();
        $key = class_exists('ZICA_AI_IndexNow') ? ZICA_AI_IndexNow::get_key() : '';
    ...
    private static function check_schema() {
        $ok = class_exists('ZICA_AI_Schema_Validator');
        return array(
            'id' => 'schema',
    ...
    private static function check_meta_auditor() {
        $enabled = class_exists('ZICA_AI_Meta_Auditor') && ZICA_AI_Meta_Auditor::is_enabled();
        return array(
            'id' => 'meta_auditor',
    ...
    private static function check_internal_links() {
        $ok = class_exists('ZICA_AI_Internal_Links');
        return array(
            'id' => 'internal_links',
    ...
    private static function check_image_optimization() {
        $ok = class_exists('ZICA_AI_Image_Optimizer');
        return array(
    ...
    private static function check_ai_traffic() {
        $enabled = class_exists('ZICA_AI_AI_Traffic_Detector') && ZICA_AI_AI_Traffic_Detector::is_enabled();
        return array(
            'id' => 'ai_traffic',
    ...
    private static function check_news_sitemap() {
        $enabled = class_exists('ZICA_AI_Sitemap_Optimizer') && ZICA_AI_Sitemap_Optimizer::is_enabled();
        if ($enabled) {
    ...
    public static function render_panel() {
        ...
        ?>
        <div class="zica-ai-card zica-ai-seo-checklist-card">
            <div class="zica-ai-card-header">
                <h2>
                    <span class="dashicons dashicons-yes-alt"></span>
                    <?php _e('Checklist Fundação SEO', 'zica-ai'); ?>
                </h2>
                <div class="zica-ai-checklist-score">
            ...
            <div class="zica-ai-card-body">
                ...
                <div class="zica-ai-checklist-group">
                    <h3 class="zica-ai-checklist-category"><?php echo esc_html($category); ?></h3>
                    <?php foreach ($items as $item): ?>
                    <div class="zica-ai-checklist-item <?php echo esc_attr($item['status']); ?>">
                        <span class="checklist-indicator">
                            <?php if ($item['status'] === 'ok'): ?>
                                <span class="dashicons dashicons-yes-alt" style="color: var(--zica-ai-success);"></span>
                            <?php elseif ($item['status'] === 'warn'): ?>
                                <span class="dashicons dashicons-warning" style="color: var(--zica-ai-warning);"></span>
                            <?php else: ?>
                                <span class="dashicons dashicons-dismiss" style="color: var(--zica-ai-danger);"></span>
                            <?php endif; ?>
                        </span>
