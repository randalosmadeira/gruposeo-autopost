...
class ZICA_AI_HTTPS_Enforcer {
    ...
    public function init() {
        ...
        // Schedule weekly scan (using custom interval from ZICA_AI_Cron_Scheduler)
    ...
    public function scan_all_posts($limit = 100) {
        ...
        ZICA_AI_Logger::info('https_scan', 'Scan concluído', array(
            'scanned' => $results['scanned'],
            'with_http' => $results['with_http'],
    ...
    public function convert_to_https($post_id, $internal_only = false) {
        ...
        if ($changes) {
            ...
            ZICA_AI_Logger::success('https_convert', 'Links HTTP convertidos', array(
                'post_id' => $post_id,
    ...
    public function bulk_update_database($internal_only = true) {
        ...
        ZICA_AI_Logger::success('https_bulk', 'Atualização em massa concluída', $results);
