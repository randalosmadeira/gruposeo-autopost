...
class ZICA_AI_Google_Indexing_Submitter {
    ...
    public function submit_url($url, $type = 'URL_UPDATED') {
        // Check daily quota
        if (!$this->check_quota()) {
            ZICA_AI_Logger::warning('google_indexing', 'Quota diária atingida (200 URLs/dia)', array('url' => $url));
            return false;
        }
        
        // Get access token from GSC integration
        if (!class_exists('ZICA_AI_GSC_Integration') || !ZICA_AI_GSC_Integration::is_connected()) {
            // Fallback: use IndexNow instead
            if (class_exists('ZICA_AI_IndexNow')) {
                ZICA_AI_IndexNow::get_instance()->submit_indexnow($url);
                ZICA_AI_Logger::info('google_indexing', 'GSC não conectado, URL submetida via IndexNow', array('url' => $url));
                return true;
            }
            ZICA_AI_Logger::warning('google_indexing', 'GSC não conectado e IndexNow indisponível');
            return false;
        }
        ...
        if ($result) {
            ...
            ZICA_AI_Logger::success('google_indexing', 'URL submetida à Google Indexing API', array(
                'url' => $url,
                'type' => $type,
            ));
    ...
    public function submit_batch($urls) {
        ...
        ZICA_AI_Logger::info('google_indexing', 'Batch submission concluído', $results);
        return $results;
    }
    ...
    public function register_routes() {
        register_rest_route('zica-ai/v1', '/google-indexing/submit', array(
            'methods' => 'POST',
        ...
        register_rest_route('zica-ai/v1', '/google-indexing/batch', array(
            'methods' => 'POST',
        ...
        register_rest_route('zica-ai/v1', '/google-indexing/status', array(
            'methods' => 'GET',
    ...
    public function rest_status() {
        ...
        return rest_ensure_response(array(
            ...
            'gsc_connected' => class_exists('ZICA_AI_GSC_Integration') && ZICA_AI_GSC_Integration::is_connected(),
            'quota' => array(
                'used' => $count,
