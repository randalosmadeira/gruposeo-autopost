...
class ZICA_AI_Diagnostics_Page {

    public static function render() {
        if (!current_user_can('manage_options')) {
            wp_die(__('Você não tem permissão para acessar esta página.', 'zica-ai'));
        }
        ...
        $report = ZICA_AI_Diagnostics::get_report();
        $status = $report['status'] ?? 'warning';

        $status_label = 'OK';
        $status_color = '#2e7d32';

        if ($status === ZICA_AI_Diagnostics::STATUS_WARNING) {
            $status_label = 'AVISOS';
            $status_color = '#b26a00';
        } elseif ($status === ZICA_AI_Diagnostics::STATUS_CRITICAL) {
            $status_label = 'CRÍTICO';
            $status_color = '#c62828';
        }

        $health_url = esc_url(rest_url('zica-ai/v1/healthcheck'));
        $basic_health_url = esc_url(rest_url('zica-ai/v1/health'));

        echo '<div class="wrap zica-ai-wrap">';
    ...
    private static function render_tables_section() {
        ...
        if (!empty($missing_tables)) {
            ...
            echo '<button type="button" id="zica-ai-repair-tables" class="button button-primary" onclick="cfrdmRepairTables()">';
            echo '<span class="dashicons dashicons-chart-area" style="vertical-align:middle;margin-right:5px;"></span>';
            echo 'Reparar Tabelas e Gerar API Key</button>';
            echo '<span id="zica-ai-repair-status" style="margin-left:10px;"></span>';
            echo '</div>';
        }
        ...
        function cfrdmRepairTables() {
            var btn = document.getElementById("zica-ai-repair-tables");
            var status = document.getElementById("zica-ai-repair-status");
            ...
            fetch("' . esc_url(rest_url('zica-ai/v1/repair-tables')) . '", {
                method: "POST",
                headers: {
            ...
            .then(function(data) {
                if (data.success) {
                    ...
                    btn.disabled = false;
                    btn.innerHTML = "<span class=\"dashicons dashicons-chart-area\" style=\"vertical-align:middle;margin-right:5px;\"></span>Tentar Novamente";
                }
            })
            .catch(function(e) {
        ...
        if (empty($missing_tables)) {
            ...
            echo '<button type="button" id="zica-ai-repair-tables" class="button" onclick="cfrdmRepairTables()">';
            echo '<span class="dashicons dashicons-chart-area" style="vertical-align:middle;margin-right:5px;"></span>';
    ...
    private static function render_gsc_status_section() {
        ...
        if (!$gsc_status['connected']) {
            ...
            echo '<li>Adicione o Client ID e Client Secret nas <a href="' . admin_url('admin.php?page=zica-ai-settings') . '">Configurações</a></li>';
            echo '</ol>';
            echo '</div>';
        }
    }
    ...
    private static function render_fix_queue_section() {
        ...
        $stats = array(
            ...
            'Falharam' => $failed ?? 0,
            'Auto-Fix habilitado' => ZICA_AI_AI_Auto_Fix::is_enabled() ? '✓ Sim' : '✗ Não',
            'Confiança mínima' => (ZICA_AI_AI_Auto_Fix::get_min_confidence() <= 1 ? round(ZICA_AI_AI_Auto_Fix::get_min_confidence() * 100) : round(ZICA_AI_AI_Auto_Fix::get_min_confidence())) . '%',
        );
