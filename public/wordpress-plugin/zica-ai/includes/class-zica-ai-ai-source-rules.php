...
class ZICA_AI_AI_Source_Rules {
    ...
    public function detect_ai_referral() {
        ...
        ZICA_AI_Logger::info('ai_source', 'Visita via IA detectada', array(
            'fonte' => $matched_rule['fonte'],
            'url' => $current_url,
    ...
    private function auto_submit_for_indexing($url) {
        ...
        if (class_exists('ZICA_AI_Google_Indexing_Submitter')) {
            try {
                ZICA_AI_Google_Indexing_Submitter::get_instance()->submit_url($url);
            } catch (\Throwable $e) {
                // Silent fail
            }
        }
    ...
    public function register_routes() {
        register_rest_route('zica-ai/v1', '/ai-source-rules', array(
            array(
                'methods' => 'GET',
        ...
        register_rest_route('zica-ai/v1', '/ai-source-stats', array(
            'methods' => 'GET',
