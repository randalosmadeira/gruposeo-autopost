 ...
 * - ZICA_AI_Google_Indexing_Submitter (Google Indexing API)
 * - ZICA_AI_IndexNow (IndexNow + Bing/Yandex ping)
...
class ZICA_AI_Instant_Indexing {
    ...
    public function add_admin_bar_menu($wp_admin_bar) {
        ...
        $wp_admin_bar->add_node(array(
            'id' => 'zica-ai-instant-indexing',
            'title' => '<span class="ab-icon dashicons dashicons-admin-site-alt3"></span> IndexNow <span class="zica-ai-quota-badge">' . $quota['remaining'] . '</span>',
            'href' => '#',
            'meta' => array(
                'class' => 'zica-ai-instant-indexing-bar',
        ...
        if (is_singular()) {
            $wp_admin_bar->add_node(array(
                'parent' => 'zica-ai-instant-indexing',
                'id' => 'zica-ai-index-current',
                'title' => '⚡ Indexar Esta Página',
                'href' => '#',
                'meta' => array(
                    'class' => 'zica-ai-index-current-page',
        ...
        $wp_admin_bar->add_node(array(
            'parent' => 'zica-ai-instant-indexing',
            'id' => 'zica-ai-index-recent',
            'title' => '📋 Indexar Últimos 50 Posts',
            'href' => admin_url('admin.php?page=zica-ai-settings&tab=indexing'),
        ));

        // Sub-item: Quota status
        $wp_admin_bar->add_node(array(
            'parent' => 'zica-ai-instant-indexing',
            'id' => 'zica-ai-quota-status',
            'title' => sprintf('📊 Quota: %d/%d usados', $quota['used'], self::MAX_DAILY_QUOTA),
            'href' => admin_url('admin.php?page=zica-ai-diagnostics'),
        ));
    }
    ...
    public function render_indexing_column($column, $post_id) {
        ...
        if ($post->post_status !== 'publish') {
            echo '<span class="zica-ai-idx-status zica-ai-idx-na" title="Não publicado">—</span>';
            return;
        }
        ...
        if ($last_indexed) {
            ...
            echo '<span class="zica-ai-idx-status zica-ai-idx-ok" title="Indexado há ' . esc_attr($time_ago) . '">✅</span>';
            echo '<br><small style="color:#666;font-size:10px;">' . esc_html($time_ago) . '</small>';
        } else {
            echo '<a href="#" class="zica-ai-idx-push" data-post-id="' . esc_attr($post_id) . '" title="Clique para indexar agora">';
            echo '<span class="zica-ai-idx-status zica-ai-idx-pending">⏳</span>';
            echo '</a>';
        }
    }
    ...
    public function bulk_action_notices() {
        if (!empty($_REQUEST['cfrdm_indexed'])) {
            ...
            $msg = sprintf(
                '⚡ Zica.ai: %d URL(s) submetidas para indexação.',
                $indexed
            );
            if ($failed > 0) {
    ...
    public function submit_for_indexing($url, $post_id = null) {
        if (!$this->check_quota()) {
            ZICA_AI_Logger::warning('instant_indexing', 'Quota diária atingida', array('url' => $url));
            return false;
        }
        ...
        if (class_exists('ZICA_AI_IndexNow')) {
            try {
                ...
                ZICA_AI_Logger::warning('instant_indexing', 'IndexNow falhou: ' . $e->getMessage());
            }
        }
        ...
        if (class_exists('ZICA_AI_Google_Indexing_Submitter')) {
            try {
                $google = ZICA_AI_Google_Indexing_Submitter::get_instance();
                ...
                ZICA_AI_Logger::warning('instant_indexing', 'Google API falhou: ' . $e->getMessage());
            }
        }
        ...
        if ($submitted) {
            ...
            ZICA_AI_Logger::success('instant_indexing', 'URL indexada via: ' . implode(', ', $channels), array(
                'url' => $url,
                'post_id' => $post_id,
                'channels' => $channels,
            ), $post_id);
    ...
    public function enqueue_admin_scripts($hook) {
        ...
        jQuery(document).ready(function($) {
            ...
            $(document).on('click', '.zica-ai-idx-push', function(e) {
                e.preventDefault();
                var btn = $(this);
                ...
                $.post(ajaxurl, {
                    ...
                    if (res.success) {
                        btn.replaceWith('<span class=\"zica-ai-idx-status zica-ai-idx-ok\" title=\"Indexado agora!\">✅</span><br><small style=\"color:#666;font-size:10px;\">agora</small>');
                    } else {
                        btn.html('<span class=\"zica-ai-idx-status zica-ai-idx-error\" title=\"Erro\">❌</span>');
                    }
                });
            });
        });
        ...
        function cfrdmInstantIndexCurrent() {
            ...
            jQuery.post('" . admin_url('admin-ajax.php') . "', {
                ...
                if (res.success) {
                    ...
                    // Update quota badge
                    jQuery('.zica-ai-quota-badge').text(res.data.quota.remaining);
                } else {
    ...
    public function admin_bar_inline_css() {
        ...
        .zica-ai-idx-status { cursor: default; font-size: 14px; }
        .zica-ai-idx-push { text-decoration: none; }
        .zica-ai-idx-push:hover .zica-ai-idx-pending { opacity: 0.7; }
        #wp-admin-bar-zica-ai-instant-indexing .ab-icon { top: 2px; }
    ...
    public function register_routes() {
        register_rest_route('zica-ai/v1', '/instant-indexing/submit', array(
            'methods' => 'POST',
            'callback' => array($this, 'rest_submit'),
            'permission_callback' => array('ZICA_AI_API', 'verify_api_key'),
        ));

        register_rest_route('zica-ai/v1', '/instant-indexing/batch', array(
            'methods' => 'POST',
            'callback' => array($this, 'rest_batch'),
            'permission_callback' => array('ZICA_AI_API', 'verify_api_key'),
        ));

        register_rest_route('zica-ai/v1', '/instant-indexing/status', array(
            'methods' => 'GET',
...
add_action('cfrdm_delayed_index', function($post_id) {
    if (!class_exists('ZICA_AI_Instant_Indexing')) return;
    $url = get_permalink($post_id);
    if ($url) {
        ZICA_AI_Instant_Indexing::get_instance()->submit_for_indexing($url, $post_id);
    }
});
