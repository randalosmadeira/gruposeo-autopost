...
class ZICA_AI_GMB_Poster {
    ...
    public function post_to_gmb($post_id) {
        ...
        if (!$access_token) {
            ZICA_AI_Logger::error('gmb', 'GMB não conectado');
            return false;
        }
        ...
        if (empty($account_id) || empty($location_id)) {
            ZICA_AI_Logger::error('gmb', 'Account/Location ID não configurados');
            return false;
        }
        ...
        if (is_wp_error($response)) {
            ZICA_AI_Logger::error('gmb', 'Erro ao postar no GMB', array(
        ...
        if ($status_code >= 400) {
            ...
            ZICA_AI_Logger::error('gmb', 'GMB API retornou erro', array(
                'status' => $status_code,
                'error' => $error,
                'post_id' => $post_id,
            ));
        ...
        ZICA_AI_Logger::success('gmb', 'Post publicado no Google Meu Negócio', array(
            'post_id' => $post_id,
            'gmb_name' => $body['name'] ?? '',
        ));
    ...
    public function handle_oauth_callback() {
        if (!isset($_GET['page']) || $_GET['page'] !== 'zica-ai-gmb-auth') return;
        if (!isset($_GET['code'])) return;
        
        $code = sanitize_text_field($_GET['code']);
        $client_id = get_option('cfrdm_gmb_client_id', get_option('cfrdm_gsc_client_id'));
        $client_secret = get_option('cfrdm_gmb_client_secret', get_option('cfrdm_gsc_client_secret'));
        $redirect_uri = admin_url('admin.php?page=zica-ai-gmb-auth');
        ...
        if (!is_wp_error($response)) {
            ...
            if (isset($body['access_token'])) {
                $body['created'] = time();
                update_option(self::OPTION_ACCESS_TOKEN, $body);
                ZICA_AI_Logger::success('gmb', 'Google Meu Negócio conectado com sucesso');
                wp_redirect(admin_url('admin.php?page=zica-ai-settings&gmb_connected=1'));
                exit;
            }
        }
        
        wp_redirect(admin_url('admin.php?page=zica-ai-settings&gmb_error=1'));
        exit;
    }
    ...
    public function register_routes() {
        register_rest_route('zica-ai/v1', '/gmb/status', array(
            'methods' => 'GET',
            'callback' => array($this, 'rest_status'),
            'permission_callback' => function() { return ZICA_AI_API::verify_api_key(); },
        ));
        
        register_rest_route('zica-ai/v1', '/gmb/post', array(
            'methods' => 'POST',
            'callback' => array($this, 'rest_post'),
            'permission_callback' => function() { return ZICA_AI_API::verify_api_key(); },
        ));
        
        register_rest_route('zica-ai/v1', '/gmb/history', array(
            'methods' => 'GET',
    ...
    public function disconnect() {
        ...
        ZICA_AI_Logger::info('gmb', 'Google Meu Negócio desconectado');
    }
}
