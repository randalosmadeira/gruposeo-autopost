...
class ZICA_AI_Auto_Update {
    ...
    const BACKUP_DIR = 'zica-ai-backups';
    
    // Zica.ai update server URLs
    const UPDATE_API_URL = 'https://raw.githubusercontent.com/randalosmadeira/gruposeo-autopost/main/public/wordpress-plugin/zica-ai/version.json';
    const PLUGIN_DOWNLOAD_URL = 'https://github.com/randalosmadeira/gruposeo-autopost/releases/latest/download/zica-ai.zip';
    ...
    public function check_for_updates() {
        ...
        if ($update_info) {
            ...
            ZICA_AI_Logger::info('auto_update', 'Atualização disponível', array(
                'current' => ZICA_AI_VERSION,
            ...
            if (self::is_enabled()) {
                ...
                if ($safety['safe']) {
                    ...
                    ZICA_AI_Logger::warning('auto_update', 'Atualização bloqueada por verificação de segurança', $safety);
                }
            }
    ...
    public function run_pre_update_checks($update_info) {
        ...
        $free_space = @disk_free_space(ZICA_AI_PLUGIN_DIR);
    ...
    private function check_remote_api() {
        $response = wp_remote_get(self::UPDATE_API_URL . '?current_version=' . ZICA_AI_VERSION, array(
            'timeout' => 15,
            'headers' => array(
        ...
        if (is_wp_error($response)) {
            ZICA_AI_Logger::warning('auto_update', 'Erro ao verificar atualizações via API', array(
        ...
        if ($latest && version_compare($latest, ZICA_AI_VERSION, '>')) {
            return array(
                'version' => $latest,
     ...
     * Compares against version.json in the Zica.ai server
     */
    private function check_local_manifest() {
        ...
        if (version_compare($manifest['version'], ZICA_AI_VERSION, '>')) {
            return array(
    ...
    public function show_update_notice() {
        ...
        if ($current_screen && strpos($current_screen->id, 'cfrdm') !== false) {
            ...
            <div class="notice notice-info is-dismissible">
                <p>
                    <strong>Zica.ai <?php echo esc_html($update_info['version']); ?> disponível!</strong>
                    Versão atual: <?php echo esc_html(ZICA_AI_VERSION); ?>
                </p>
                <p>
                    <?php if (self::is_enabled()): ?>
                        A atualização será aplicada automaticamente.
                    <?php else: ?>
                        <a href="<?php echo esc_url(admin_url('admin.php?page=zica-ai-settings&tab=updates')); ?>" class="button button-primary">
                            Atualizar Agora
                        </a>
                    <?php endif; ?>
                </p>
    ...
    public function create_backup() {
        ...
        $backup_name = 'zica-ai-backup-' . ZICA_AI_VERSION . '-' . date('Ymd-His') . '.zip';
        $backup_path = $backup_dir . '/' . $backup_name;
        
        // Create ZIP backup
        if (!class_exists('ZipArchive')) {
            ZICA_AI_Logger::error('auto_update', 'ZipArchive não disponível');
            return false;
        }
        ...
        if ($zip->open($backup_path, ZipArchive::CREATE) !== true) {
            ZICA_AI_Logger::error('auto_update', 'Erro ao criar arquivo de backup');
            return false;
        }
        
        // Add plugin files to backup
        $plugin_dir = ZICA_AI_PLUGIN_DIR;
        $this->add_folder_to_zip($zip, $plugin_dir, 'zica-ai');
        ...
        ZICA_AI_Logger::success('auto_update', 'Backup criado', array(
            'path' => $backup_path,
            'version' => ZICA_AI_VERSION,
        ));
    ...
    public function apply_update_patch() {
        ...
        try {
            ...
            // Notify Zica.ai platform about successful update
            $this->notify_platform_update($update_info);
            
            ZICA_AI_Logger::success('auto_update', 'Atualização aplicada com sucesso', array(
                'from' => ZICA_AI_VERSION,
                'to' => $update_info['version'],
            ));
            
            return array(
                'success' => true,
                'from_version' => ZICA_AI_VERSION,
            ...
            ZICA_AI_Logger::error('auto_update', 'Falha na atualização - rollback executado', array(
                'error' => $e->getMessage(),
            ));
    ...
    private function verify_plugin_integrity() {
        $main_file = ZICA_AI_PLUGIN_DIR . 'zica-ai.php';
        ...
        $required_files = array(
            'includes/class-zica-ai-logger.php',
            'includes/class-zica-ai-api.php',
            'includes/class-zica-ai-seo.php',
        );
        
        foreach ($required_files as $file) {
            if (!file_exists(ZICA_AI_PLUGIN_DIR . $file)) {
    ...
    /**
     * Notify Zica.ai platform about this site's update status
     */
    private function notify_platform_update($update_info) {
        ...
        $payload = array(
            ...
            'previous_version' => ZICA_AI_VERSION,
    ...
    private function apply_patch($patch_url) {
        ...
        foreach ($patch_data['files'] as $file_path => $file_data) {
            $full_path = ZICA_AI_PLUGIN_DIR . $file_path;
    ...
    private function apply_full_update($download_url) {
        ...
        $plugin_dir = dirname(ZICA_AI_PLUGIN_DIR);
    ...
    public function run_database_migration($version) {
        ...
        ZICA_AI_Logger::info('auto_update', 'Migração de banco de dados executada', array(
            'version' => $version,
        ));
    }
    ...
    public function rollback_update($backup_path = null) {
        ...
        $this->remove_directory(ZICA_AI_PLUGIN_DIR);
        ...
        ZICA_AI_Logger::success('auto_update', 'Rollback executado', array(
            'backup' => $backup_path,
        ));
    ...
    public function notify_admin($update_info) {
        ...
        $subject = 'Zica.ai atualizado para v' . $update_info['version'];
        
        $message = "O plugin Zica.ai foi atualizado automaticamente.\n\n";
        $message .= "Versão anterior: " . ZICA_AI_VERSION . "\n";
        ...
        $message .= admin_url('admin.php?page=zica-ai-dashboard');
