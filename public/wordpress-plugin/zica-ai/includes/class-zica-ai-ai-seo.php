...
class ZICA_AI_AI_SEO {
    ...
    public static function generate($post_id, $options = array()) {
        ...
        if (!$post) {
            return new WP_Error('invalid_post', __('Post não encontrado.', 'zica-ai'));
        }
        ...
        ZICA_AI_Logger::success('ai_seo', 'SEO gerado com sucesso via IA', array(
            'post_id' => $post_id,
            'slug' => $seo_data['slug'],
    ...
    private static function call_ai_api($prompt) {
        ...
        if (empty($api_endpoint)) {
            return new WP_Error('no_api', __('API URL não configurada.', 'zica-ai'));
        }
        
        $api_key = get_option('zica_ai_api_key');
        
        if (empty($api_key)) {
            return new WP_Error('no_api_key', __('API Key não configurada.', 'zica-ai'));
        }
        ...
        if (is_wp_error($response)) {
            ZICA_AI_Logger::error('ai_seo', 'Erro ao chamar API de IA', array(
    ...
    private static function parse_ai_response($response) {
        ...
        if ($json_str === null) {
            return new WP_Error('invalid_response', __('A IA não retornou um JSON válido.', 'zica-ai'));
        }
        
        $data = json_decode($json_str, true);
        
        if (json_last_error() !== JSON_ERROR_NONE || !is_array($data)) {
            return new WP_Error('json_error', __('Falha ao interpretar o JSON retornado.', 'zica-ai'));
        }
    ...
    public static function apply_to_post($post_id, $seo_data) {
        ...
        if (class_exists('ZICA_AI_SEO')) {
            ...
            if (!empty($seo_data['meta_description'])) {
                ZICA_AI_SEO::set_meta_description($post_id, $seo_data['meta_description']);
            }
            
            // Focus keyword
            if (!empty($seo_data['focus_keyword'])) {
                ZICA_AI_SEO::set_focus_keyword($post_id, $seo_data['focus_keyword']);
            }
        }
        ...
        ZICA_AI_Logger::success('ai_seo', 'SEO aplicado ao post', array(
            'post_id' => $post_id,
            'slug' => $seo_data['slug'],
        ), $post_id);
    }
