...
final class Zica_Electoral_Analytics {
    ...
    private static function config(bool $refresh = false): array {
        ...
        $response = wp_remote_get($url, [
            'timeout' => 4,
            'redirection' => 2,
            'headers' => ['Accept' => 'application/json', 'User-Agent' => 'Zica-Electoral-Analytics/' . self::VERSION],
        ]);
    ...
    public static function render_google_loader(): void {
        $config = self::config();
        if (!self::active($config) || empty($config['analytics_enabled'])) return;
        ...
        if ($gtm !== '') {
            ...
            $params = ['allow_google_signals' => false, 'allow_ad_personalization_signals' => false, 'send_page_view' => true];
    ...
    public static function enqueue_frontend(): void {
        ...
        if (!empty($config['analytics_enabled'])) {
            wp_enqueue_script('zica-electoral-analytics', plugins_url('assets/zica-electoral-analytics.js', __FILE__), [], self::VERSION, true);
        ...
        if (!empty($config['optin_enabled']) && !empty($config['optin_api_url'])) {
            wp_enqueue_style('zica-electoral-optin', plugins_url('assets/zica-electoral-optin.css', __FILE__), [], self::VERSION);
            wp_enqueue_script('zica-electoral-optin', plugins_url('assets/zica-electoral-optin.js', __FILE__), [], self::VERSION, true);
            wp_localize_script('zica-electoral-optin', 'ZicaElectoralOptin', [
                ...
                'privacy' => ['browsingHistoryLinkedToContact' => false, 'personalizedPoliticalTargeting' => false],
            ]);
        }
    }
}
