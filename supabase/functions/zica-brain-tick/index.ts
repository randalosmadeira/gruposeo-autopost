  ...
  if (openaiKey) {
    try {
      const r = await fetch("https://api.openai.com/v1/responses", {
        ...
        body: JSON.stringify({ model: "gpt-5.6-sol", input: "Responda somente OK.", max_output_tokens: 16, store: false }),
        signal: AbortSignal.timeout(30000),
      });
      const text = await r.text();
      let payload: any = {}; try { payload = JSON.parse(text); } catch { /* ignore */ }
      ...
      result.openai = { configured: true, operational: false, httpStatus: 0, model: "gpt-5.6-sol", errorCode: "network_error", detail: e instanceof Error ? e.message : "error" };
    }
  ...
  if (anthropicKey) {
    try {
      const r = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({ model: "claude-sonnet-4-6", max_tokens: 8, messages: [{ role: "user", content: "Responda somente OK." }] }),
        signal: AbortSignal.timeout(30000),
      });
      ...
      result.anthropic = { configured: true, operational: false, httpStatus: 0, model: "claude-sonnet-4-6", errorType: "network_error", detail: e instanceof Error ? e.message : "error" };
    }
  } else result.anthropic = { configured: false, operational: false, httpStatus: null };

  for (const provider of ["openai", "anthropic"] as const) {
    await admin.from("zica_ai_provider_health_cache").upsert({ provider, checked_at: result.checked_at, payload: { provider: result[provider], operational: result[provider]?.operational === true, billable_probe: true } }, { onConflict: "provider" });
  }
  return result;
}
    ...
    for (const job of jobs || []) {
      ...
      try {
        if (job.job_type === "wordpress_reconcile") {
          ...
          const open = result.openai?.operational === true, claude = result.anthropic?.operational === true;
          ok = open || claude;
