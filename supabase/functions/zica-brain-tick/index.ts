...
function bucket(minutes: number) { const ms = minutes * 60_000; return new Date(Math.floor(Date.now() / ms) * ms).toISOString(); }
...
async function state(admin: any, userId: string, subsystem: string, status: "healthy"|"degraded"|"offline"|"unknown", metrics: Record<string, unknown> = {}, error?: string) {
  ...
  await admin.from("zica_brain_state").upsert({
    user_id: userId, subsystem, status, last_heartbeat_at: now,
...
async function edgeCall(baseUrl: string, serviceKey: string, slug: string, body: Record<string, unknown>, timeoutMs = 90000) {
  ...
  let data: any = null; try { data = JSON.parse(text); } catch { data = { error: text.slice(0, 800) }; }
...
async function providerHealth(admin: any, userId: string) {
  ...
  if (anthropicKey) {
    try {
      ...
      const payload = await r.json().catch(() => ({})); result.anthropic = { configured: true, operational: r.ok, httpStatus: r.status, model: "claude-sonnet-4-6", errorType: payload?.error?.type || null };
...
async function auditArticle(admin: any, userId: string, articleId: string) {
  ...
  const html = String(article.content || ""), text = stripHtml(html), words = text.split(/\s+/).filter(Boolean);
  const h2 = countMatches(html, /<h2\b/gi) + countMatches(html, /^##\s+/gm), h3 = countMatches(html, /<h3\b/gi) + countMatches(html, /^###\s+/gm);
  ...
  const faq = /FAQ|perguntas frequentes|<details|FAQPage/i.test(html), hasExcerpt = String(article.excerpt || "").trim().length >= 80, hasKeyword = String(article.keyword || "").trim().length > 2, hasTitle = String(article.title || "").trim().length > 10;
  let discovery = 0;
  if (article.project_id) {
    ...
    const serialized = JSON.stringify(stats?.raw_data || {}); if (/llms\.txt/i.test(serialized)) discovery += 5; if (/ai\.txt/i.test(serialized)) discovery += 4; if (/sitemap/i.test(serialized)) discovery += 4; if (/schema/i.test(serialized)) discovery += 4; if (/rss|feed/i.test(serialized)) discovery += 4;
  }
  ...
  const now = new Date().toISOString(), traffic = article.status === "published" ? "active" : article.status === "error" ? "error" : undefined;
...
async function syncProjectRss(admin: any, userId: string, projectId: string) {
  const { data: project } = await admin.from("projects").select("id,wordpress_url,rss_feed_url").eq("id", projectId).eq("user_id", userId).maybeSingle();
  if (!project?.wordpress_url) throw new Error("project_wordpress_url_missing");
  const discovery = await discoverFeed(String(project.wordpress_url), { directCandidate: project.rss_feed_url || null });
  if (!discovery.valid) throw new Error(`project_rss_not_found:${discovery.reason || "unknown"}`);
  const validatedAt = new Date().toISOString();
  const validation = { url: discovery.url, format: discovery.format, http_status: discovery.status, content_type: discovery.content_type, content_type_valid: discovery.content_type_valid, structure_valid: discovery.structure_valid, discovery_method: discovery.discovery_method, attempted: discovery.attempted };
  await admin.from("projects").update({ rss_feed_url: discovery.url, rss_feed_validation: validation, rss_feed_validated_at: validatedAt, updated_at: validatedAt }).eq("id", projectId).eq("user_id", userId);
  await admin.from("articles").update({ rss_feed_url: discovery.url, updated_at: validatedAt }).eq("project_id", projectId).eq("user_id", userId).is("rss_feed_url", null);
  return validation;
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!automationKey || !ingress?.enabled || !ingress.secret_hash || await sha256(automationKey) !== ingress.secret_hash) return json({ success: false, error: "automation_unauthorized", request_id: requestId }, 401);
    const body = await req.json().catch(() => ({})), maxJobs = Math.max(1, Math.min(50, Number(body?.maxJobs || 20)));
    ...
    for (const userId of users) {
      ...
      const { data: projects } = await admin.from("projects").select("id,wordpress_url").eq("user_id", userId).eq("is_connected", true);
      for (const project of projects || []) {
        ...
        if (project.wordpress_url) { await enqueue(admin, { user_id: userId, project_id: project.id, job_type: "rss_discovery", status: "queued", priority: 70, idempotency_key: `rss:${project.id}:${sixHours}`, payload: { projectId: project.id }, next_attempt_at: new Date().toISOString() }); enqueued++; }
      }
    ...
    for (const job of jobs || []) {
      ...
      try {
        if (job.job_type === "wordpress_reconcile") {
          const call = await edgeCall(url, serviceKey, "wordpress-operations", { userId: job.user_id, projectId: job.project_id, action: "process_due" }, 90000); ok = call.ok; result = call.data; errorMessage = call.ok ? "" : String(call.data?.error || `HTTP ${call.status}`); await state(admin, job.user_id, "wordpress", ok ? "healthy" : "degraded", { projectId: job.project_id, httpStatus: call.status }, errorMessage || undefined);
        } else if (job.job_type === "scheduled_publish") {
          const call = await edgeCall(url, serviceKey, "wordpress-operations", { userId: job.user_id, articleId: job.article_id, projectId: job.project_id, action: "publish", publishStatus: "publish", requireFeaturedImage: false }, 90000); ok = call.ok; result = call.data; errorMessage = call.ok ? "" : String(call.data?.error || `HTTP ${call.status}`); await state(admin, job.user_id, "scheduler", ok ? "healthy" : "degraded", { articleId: job.article_id, routed_via: "wordpress_operations" }, errorMessage || undefined);
        } else if (job.job_type === "rss_discovery") {
          result = await syncProjectRss(admin, job.user_id, job.project_id); ok = true; await state(admin, job.user_id, "rss_discovery", "healthy", { projectId: job.project_id, feed: result.url, discovery_method: result.discovery_method });
        } else if (job.job_type === "provider_health") {
          result = await providerHealth(admin, job.user_id); const open = result.openai?.operational === true, claude = result.anthropic?.operational === true; ok = open || claude; await state(admin, job.user_id, "ai_providers", open && claude ? "healthy" : ok ? "degraded" : "offline", { openai: result.openai, anthropic: result.anthropic }, ok ? undefined : "Nenhum provedor textual operacional");
      ...
      await admin.rpc("finish_zica_brain_job", { p_id: job.id, p_ok: ok, p_result: result || {}, p_error: errorMessage || null }); results.push({ id: job.id, type: job.job_type, ok, error: errorMessage || null });
    }
    for (const userId of users) {
