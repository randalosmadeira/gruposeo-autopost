...
function choosePortalResources(keyword: string, electoral: ElectoralConfig, settings: PortalSettings | null, resources: PortalResource[]) {
  ...
  const coverageIntent = hasAnyToken(contextTokens, ['madeira','1470','candidato','candidatos','candidatura','registro','perfil','lista','eleicoes','eleicao','deputado','missao']);
  const partyDirectoryIntent = hasAnyToken(contextTokens, ['missao','partido','candidato','candidatos','lista','chapa','nomes']);
  ...
  const ranked: RankedResource[] = resources.map((resource) => {
    ...
    return { resource, matches, isPrimary, score: Number(resource.priority || 0) + matches * 25 + (isPrimary ? 1000 : 0) };
  ...
  try {
    ...
    return new Response(JSON.stringify({
      ...
      audit: {
        ...
        safe_corpus_enabled: true,
        safe_corpus_count: corpusUnits.length,
        blocked_archive_content_excluded: true,
