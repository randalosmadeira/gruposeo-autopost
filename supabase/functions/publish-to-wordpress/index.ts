...
async function uploadPluginImage(baseUrl: string, apiKey: string, dataUrl: string, article: Record<string, any>) {
  ...
  const semanticBase = String(imageGeo.semantic_filename || article.slug || "featured")
    .replace(/\.[a-z0-9]+$/i, "")
    .replace(/[^a-zA-Z0-9-]+/g, "-")
    .toLowerCase();
...
async function publishPlugin(
  ...
  const payload: Record<string, unknown> = {
    ...
    seo_description: String(config.seo_description || article.excerpt || ""),
  ...
  return {
    ...
    pluginContract: "zica-posts",
    pluginNamespace: project.wordpress_plugin_namespace || "zica-posts/v1",
...
async function standardRequest(baseUrl: string, path: string, auth: string, init: RequestInit) {
  ...
  for (const url of urls) {
    const response = await fetch(url, {
      ...init,
      headers: { ...(init.headers || {}), Authorization: `Basic ${auth}`, Accept: "application/json" },
    });
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!body.articleId || !body.projectId) {
      return json({ success: false, error: "articleId e projectId são obrigatórios", request_id: requestId }, 400);
    }
    ...
    if (article.project_id && article.project_id !== body.projectId && body.allowCrossProject !== true) {
      return json({
        success: false,
        error: "Artigo pertence a outro projeto. Para publicação multissite use allowCrossProject.",
    ...
    if (status === "publish" && (config.needs_primary_source || config.review_pass === false)) {
      return json({ success: false, error: "Publicação bloqueada: revisão/fonte primária pendente", code: "editorial_gate", retryable: false, request_id: requestId }, 409);
    }
    ...
    if (residues.length > 0) {
      return json({
        success: false,
        error: "Publicação bloqueada: o conteúdo contém mensagens internas, rascunhos, placeholders ou erros operacionais.",
        code: "publication_residue_gate",
        residues,
        retryable: false,
        request_id: requestId,
      }, 409);
    ...
    if (!audit.pass) {
      return json({
        success: false,
        error: "Publicação bloqueada: HTML editorial inválido",
        code: "editorial_gate",
        reason: "content_format",
        fatal_issues: audit.fatalIssues,
        issues: audit.issues,
    ...
    if (status === "publish" && words < minimum) {
      return json({ success: false, error: `Conteúdo insuficiente para publicação: ${words} palavras, mínimo configurado ${minimum}`, code: "content_gate", retryable: false, request_id: requestId }, 409);
    }
    if (status === "publish" && words >= 500 && audit.metrics.h2Count === 0) {
      return json({ success: false, error: "Publicação bloqueada: matéria longa sem estrutura H2", code: "editorial_gate", reason: "heading_structure", metrics: audit.metrics, retryable: false, request_id: requestId }, 409);
    }
    if (status === "publish" && words >= 700 && audit.metrics.strongCount === 0) {
      return json({ success: false, error: "Publicação bloqueada: matéria longa sem destaques semânticos", code: "editorial_gate", reason: "semantic_emphasis", metrics: audit.metrics, retryable: false, request_id: requestId }, 409);
    }

    const metaDescription = resolveMetaDescription({
      title: article.title,
      content: audit.html,
      excerpt: article.excerpt || config.seo_description,
    });
    if (!metaDescription) {
      return json({
        success: false,
        error: "Publicação bloqueada: não foi possível gerar uma meta description editorial válida.",
        code: "meta_description_gate",
        retryable: false,
        request_id: requestId,
      }, 409);
    }
    ...
    const { error: normalizeError } = await admin.from("articles").update({
      content: audit.html,
      excerpt: metaDescription,
      word_count: words,
      config: normalizedConfig,
    ...
    if (pluginMode) {
      ...
      result = await publishPlugin(project, normalizedArticle, credential.apiKey, status, categories, tags);
    } else {
      if (!project.wordpress_username || !project.wordpress_app_password) {
        return json({ success: false, error: "Credenciais WordPress não configuradas", request_id: requestId }, 400);
      }
