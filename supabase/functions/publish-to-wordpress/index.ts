...
type PublishRequest = { articleId:string; projectId:string; userId?:string; publishStatus?:"draft"|"publish"; requireFeaturedImage?:boolean };
...
function cleanContent(content:string){return content.replace(/<script[^>]*type\s*=\s*["']application\/ld\+json["'][^>]*>[\s\S]*?<\/script>/gi,"").replace(/<!--[\s\S]*?-->/g,"").trim();}
function endpointCandidates(baseUrl:string,namespace:string,path:string){const base=baseUrl.replace(/\/$/,""),clean=path.replace(/^\/+/,"");return[`${base}/wp-json/${namespace}/${clean}`,`${base}/?rest_route=/${namespace}/${clean}`];}
  ...
  for(const contract of contracts){
    for(const endpoint of endpointCandidates(baseUrl,contract.namespace,path)){
      try{
        ...
        const text=await response.text();let data:Record<string,any>|null=null;try{data=JSON.parse(text)}catch{}
      ...
      }catch(error){lastError=error instanceof Error?error.message:lastError;if(lastError.includes("API Key recusada"))throw new Error(lastError);}
    }
  }
...
async function resolvePluginKey(admin:any,project:Record<string,any>){
  ...
  if(ref){const{data,error}=await admin.rpc("get_zica_wordpress_credential",{p_ref:ref});if(error||!data)throw new Error("Credencial WordPress do Vault indisponível");return{apiKey:String(data),source:"vault"};}
  const legacy=String(project.wordpress_app_password||"").trim();if(!legacy)throw new Error("Credencial WordPress não configurada");return{apiKey:legacy,source:"legacy-project-field"};
}

async function uploadPluginImage(baseUrl:string,apiKey:string,dataUrl:string,slug:string,alt:string){
  if(!dataUrl.startsWith("data:image"))return undefined;
  const{data}=await pluginRequest(baseUrl,apiKey,"media",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({image_data:dataUrl,filename:`${slug||"featured"}.png`,alt_text:alt}),signal:AbortSignal.timeout(45000)});
...
async function publishPlugin(project:Record<string,any>,article:Record<string,any>,apiKey:string,status:"draft"|"publish"){
  ...
  const payload:Record<string,unknown>={title:String(article.title||""),content:cleanContent(String(article.content||"")),excerpt:String(article.excerpt||""),slug:String(article.slug||""),status,zica_ai_id:String(article.id),cfrdm_id:String(article.id),focus_keyword:String(config.focus_keyword||article.keyword||""),seo_title:config.seo_title||undefined,seo_description:config.seo_description||undefined};
  ...
  const row=result.data.data||{};return{postId:row.id,postUrl:row.link,pluginContract:result.contract.id,pluginNamespace:result.contract.namespace,endpointMode:result.endpointMode};
}

async function standardRequest(baseUrl:string,path:string,auth:string,init:RequestInit){
  const clean=path.replace(/^\/+/,"");const urls=[`${baseUrl}/wp-json/wp/v2/${clean}`,`${baseUrl}/?rest_route=/wp/v2/${clean}`];let last=500;
  for(const url of urls){const response=await fetch(url,{...init,headers:{...(init.headers||{}),Authorization:`Basic ${auth}`,Accept:"application/json"}});last=response.status;const text=await response.text();let data:any=null;try{data=JSON.parse(text)}catch{}if(response.ok&&data)return data;}
...
async function publishStandard(project:Record<string,any>,article:Record<string,any>,status:"draft"|"publish"){
  const baseUrl=String(project.wordpress_url).replace(/\/$/,""),auth=btoa(`${String(project.wordpress_username)}:${String(project.wordpress_app_password)}`);
  const post=await standardRequest(baseUrl,"posts",auth,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({title:String(article.title||""),content:cleanContent(String(article.content||"")),excerpt:String(article.excerpt||""),slug:String(article.slug||""),status}),signal:AbortSignal.timeout(60000)});
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const runtime=getRuntimeKeys();if(!runtime.url||!runtime.publicKey||!runtime.secretKey)return json({success:false,error:"Backend incompleto"},500);
    const caller=await resolveUserCaller(req,runtime,body.userId);const userId=caller.userId;const admin=adminClient(runtime);
    ...
    const status=body.publishStatus||"publish",config=article.config&&typeof article.config==="object"?article.config as Record<string,any>:{};
    ...
    let result:any,credentialSource:string|null=null;
    if(pluginMode){const credential=await resolvePluginKey(admin,project);credentialSource=credential.source;result=await publishPlugin(project,article,credential.apiKey,status);}else{
    ...
    const now=new Date().toISOString(),nextConfig={...config,wordpress_post_id:result.postId||null,wordpress_status:status,wordpress_last_sync_at:now};
    const articleUpdate=status==="publish"
      ?{status:"published",published_at:now,published_url:result.postUrl||null,error_message:null,traffic_wave_status:"active",scheduled_at:null,config:nextConfig,updated_at:now}
      :{status:"draft",error_message:null,traffic_wave_status:"draft",config:nextConfig,updated_at:now};
    await Promise.all([
    ...
    return json({success:true,articleId:article.id,postId:result.postId,postUrl:result.postUrl,status,pluginContract:result.pluginContract,pluginNamespace:result.pluginNamespace,endpointMode:result.endpointMode,credentialSource,internal:caller.internal});
  }catch(error){return json({success:false,error:error instanceof Error?error.message:"Falha ao publicar no WordPress"},errorStatus(error,502));}
});
