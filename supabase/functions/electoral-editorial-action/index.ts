...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'reanalyze') {
      const prompt = `Analise o rascunho abaixo sem reescrevê-lo. Retorne APENAS JSON válido, sem markdown, no formato:\n{\n  "score": 0,\n  "ready_for_human_review": false,\n  "summary": "",\n  "factual_risks": [],\n  "missing_sources": [],\n  "compliance_risks": [],\n  "technical_errors": [],\n  "structure_issues": [],\n  "recommended_corrections": []\n}\n\nTítulo: ${article.title || ''}\nPauta: ${article.keyword || ''}\nErro técnico registrado: ${article.error_message || 'nenhum'}\n\nCONTEÚDO:\n${content}`;
      const result = await callOpenAI(apiKey, baseInstructions, prompt, 3200);
      const analysis = extractJson(result.text);
      if (!analysis) throw new Error('openai_editorial_invalid_json');
      const config = {
        ...(article.config || {}),
        electoralEditorialReview: {
          ...analysis,
          reviewedAt: new Date().toISOString(),
          provider: 'openai',
          model: MODEL,
          keySource: 'zica-ai-vault',
          usage: result.usage,
    ...
    const { error: updateError } = await userClient.from('articles').update({
      content: corrected,
