...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    let listQuery = admin.from("supporter_avatar_requests")
      .select("id,supporter_name,email,whatsapp,city,state,status,source_count,generation_count,max_generations,candidate_preset_slug,output_format,consent_image_use,consent_terms,consent_public_gallery,consent_at,supporter_approved_at,created_at,updated_at,completed_at,pipeline_version", { count: "exact" })
      .order("created_at", { ascending: false }).range(from, to);

    if (status && status !== "all") listQuery = listQuery.eq("status", status);
    if (search) listQuery = listQuery.or(`supporter_name.ilike.%${search}%,email.ilike.%${search}%,whatsapp.ilike.%${search}%,city.ilike.%${search}%`);

    const activeStatuses = ["analyzing", "candidate_selected", "generating", "qa", "retry", "regenerate"];
    const [listResult, total, completed, failed, processing, needsReview, needsInput, gallery] = await Promise.all([
      listQuery,
      ...
      admin.from("supporter_avatar_requests").select("id", { count: "exact", head: true }).in("status", activeStatuses),
      admin.from("supporter_avatar_requests").select("id", { count: "exact", head: true }).eq("status", "needs_review"),
      admin.from("supporter_avatar_requests").select("id", { count: "exact", head: true }).eq("status", "needs_input"),
