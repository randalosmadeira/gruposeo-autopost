...
export async function validateFeedUrl(requestedUrl: string, options: { timeoutMs?: number; maxBytes?: number } = {}): Promise<FeedValidation> {
  ...
  try {
    const response = await fetch(requestedUrl, { method: "GET", redirect: "follow", headers: DEFAULT_HEADERS, signal: AbortSignal.timeout(timeoutMs) });
    ...
    return { valid, url: finalUrl, requested_url: requestedUrl, status: response.status, content_type: contentType, content_type_valid: contentTypeValid, structure_valid: structureValid, format, bytes, reason: valid ? undefined : (!contentTypeValid ? "invalid_content_type" : "invalid_xml_structure") };
  } catch (error) {
...
async function fetchHtml(siteUrl: string, timeoutMs: number, maxBytes: number): Promise<{ html: string; finalUrl: string }> {
  ...
  const response = await fetch(siteUrl, { method: "GET", redirect: "follow", headers: { ...DEFAULT_HEADERS, Accept: "text/html, application/xhtml+xml;q=0.9, application/rss+xml;q=0.6, application/xml;q=0.5" }, signal: AbortSignal.timeout(timeoutMs) });
...
export async function discoverFeed(siteUrl: string, options: { timeoutMs?: number; maxFeedBytes?: number; maxHtmlBytes?: number; directCandidate?: string | null } = {}): Promise<FeedDiscovery> {
  const timeoutMs = options.timeoutMs ?? 15000;
  ...
  return { valid: false, url: "", requested_url: siteUrl, status: null, content_type: null, content_type_valid: false, structure_valid: false, format: "unknown", bytes: 0, reason: "feed_not_found", discovery_method: "none", site_url: siteUrl, attempted };
}

function decodeXml(value: string): string {
  return value.replace(/<!\[CDATA\[|\]\]>/g, "").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&amp;/g, "&").replace(/&quot;/g, "\"").replace(/&#0?39;/g, "'").replace(/&#x27;/gi, "'").trim();
}
...
export function parseFeedXml(xml: string, feedUrl: string, limit = 20): FeedItem[] {
  ...
  for (const block of blocks.slice(0, Math.max(1, Math.min(100, limit)))) {
    ...
    items.push({ title, link, description: stripHtml(rawDescription), content: rawContent ? stripHtml(rawContent) : undefined, published_at: publishedAt, guid: stripHtml(firstTag(block, "guid") || firstTag(block, "id")) || undefined, author: stripHtml(firstTag(block, "author") || firstTag(block, "dc:creator") || firstTag(block, "name")) || undefined, categories, image_url: safeImage });
  }
